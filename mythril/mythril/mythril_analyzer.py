...
'''takes a contract, sets up the symbolic execution environment, runs the analysis modules, and generates a security report. 
It brings together the disassembler, the symbolic execution engine, and the detection modules.'''
class MythrilAnalyzer:
    ...
    '''Dumps the statespace of a contract to a serializable representation.'''
    ...
    '''Generates an HTML representation of the call graph for a contract.'''
    def graph_html(
        self,
    ...
    '''Executes the analysis modules on the contract and returns a report.'''
    def fire_lasers(
        self,
        ...
        for contract in self.contracts:
            ...
            try:
                ...
                '''Calls fire_lasers() (from security.py) to execute the selected analysis modules on the state space generated by the symbolic execution engine.'''
            ...
            for issue in issues:
                '''For each reported issue, calls issue.add_code_info() to retrieve source code information from the Solidity contract (if available).'''
